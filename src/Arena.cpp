...
{//populate items array from the map
	m_width = _w;
	m_height = _h;
...
void Arena::createDefaultObjects()
{
	...
	m_objects[Player1] = new Player(Vec2(4, 4)); // we use a pointer to inherit from Game Object. 
													//so we can specify things to player
}
...
void Arena::update(float _dt)
{
	...
	auto pos = p->getPos();//getting possition
	auto dir = p->getDir();//getting direction
	Vec2 newPos = pos;
	...
	if (dir.x !=0 && getItem(pos.x + dir.x, pos.y + dir.y) == ItemType::Empty)//is there an empty space to move to?
	{
		newPos.x = pos.x + dir.x;// calculate new possition in the x
	}

	if (dir.y != 0 && getItem(pos.x + dir.x, pos.y + dir.y) == ItemType::Empty)//is there an empty space to move to?
	{
		newPos.y = pos.y + dir.y;// calculate new possition in the y
	}
	p->setPos(newPos); //set position to the new position
	setItem(pos.x, pos.y, ItemType::Empty, m_objects[Empty]);//update the item in the arena
	setItem(newPos.x, newPos.y, ItemType::Player, m_objects[Player1]);//update the item in the arena
